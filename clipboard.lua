...
function reduceHistorySize(clearAll)
  if clearAll then
...
function addHistoryFromPasteboard()
  ...
  for index, uti in ipairs(contentTypes) do
    ...
    elseif uti == UTI_TYPE.PLAIN_TEXT then
      ...
      if text == nil or utf8.len(text) == 0 then
        return
      end
...
function choiceClipboard(choice)
  if choice then
    ...
    elseif choice.type == HISTORY_TYPE.TEXT then
      hs.pasteboard.setContents(choice.content)
    elseif choice.type == HISTORY_TYPE.CLEAR then
...
hs.hotkey.bind({"cmd", "alt", "ctrl"}, "v", showClipboard)